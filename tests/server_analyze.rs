...
fn x402_response_contract_fixtures_are_valid() {
    ...
    for expected in [
        ...
        "function toUiModel",
        "function postX402IntentPlan",
        "function requestLiveChallenge",
        "function finalizeLivePayment",
        "const LIVE_PRESETS",
        "function runLivePreset",
        "function applyLivePreset",
        "function runLiveReplayFlow",
        "allowlist_assets",
        "emergency_withdraw",
...
#[test]
fn api_x402_stellar_intent_plan_runs_soroban_v2_claim_rewards_template() {
    let model = intent_stellar_model_path();
    if !model.exists() {
        eprintln!(
            "api_x402_stellar_intent_plan_runs_soroban_v2_claim_rewards_template skipped: model not found at {}",
            model.display()
        );
        return;
    }
    ...
    let addr: SocketAddr = format!("127.0.0.1:{port}").parse().unwrap();
    let audit_path =
        std::env::temp_dir().join(format!("nc_x402_stellar_audit_soroban_v2_{port}.jsonl"));
    let policy_path =
        std::env::temp_dir().join(format!("nc_x402_stellar_claim_rewards_policy_{port}.json"));
    let policy_dir = std::env::temp_dir().join(format!("nc_x402_stellar_empty_policy_dir_{port}"));
    let _ = fs::remove_file(&audit_path);
    let _ = fs::remove_file(&policy_path);
  ...
  "intent_templates": {{
    ...
    fs::write(&policy_path, policy).expect("write claim rewards policy");
    ...
    assert_eq!(resp["plan"]["actions"][0]["args"]["pool"], "rewards");
    let logs = resp["logs"].as_array().cloned().unwrap_or_default();
    assert!(
        logs.iter()
            .filter_map(|v| v.as_str())
    ...
    assert!(
        logs.iter()
            .filter_map(|v| v.as_str())
            .any(|log| log.contains("soroban_deep_template: expanded=true template=claim_rewards")),
        "expected claim_rewards template expansion log"
    );
    ...
    .to_string();
    let (status, resp_body) = http_post_json(addr, "/api/x402/stellar/intent-plan", &blocked_body);
    assert_eq!(status, 402);
    let blocked_signature = format!("paid:{}", payment_challenge_id(&resp_body));

    let (status, resp_body) = http_post_json_with_headers(
        addr,
        "/api/x402/stellar/intent-plan",
        &blocked_body,
        &[("PAYMENT-SIGNATURE", blocked_signature.as_str())],
    );
    assert_eq!(status, 200);
    let resp: Value = serde_json::from_str(&resp_body).expect("json parse");
    ...
    assert_eq!(resp["decision"]["blocked"], true);
    assert_eq!(resp["decision"]["requires_approval"], false);
    assert_eq!(resp["decision"]["reason"], "intent_safety");
    assert_eq!(resp["guardrails"]["state"], "blocked");
    assert_eq!(resp["guardrails"]["exit_code"], 5);
    assert_eq!(resp["guardrails"]["reason"], "intent_safety");
    let warnings = resp["plan"]["warnings"]
        .as_array()
        .cloned()
        .unwrap_or_default();
    assert!(
        warnings
            ...
            .any(|warning| warning.contains(
                "slot_missing: ContractInvoke template claim_rewards missing arg account"
            )),
        "expected missing account warning"
    );
    let logs = resp["logs"].as_array().cloned().unwrap_or_default();
    assert!(
        logs.iter()
            .filter_map(|v| v.as_str())
            .any(|log| log.contains("x402: finalized challenge=")),
        "expected finalized x402 log for blocked path"
    );
    assert!(
        logs.iter()
            .filter_map(|v| v.as_str())
            .any(|log| log == "block: intent_safety"),
        "expected intent safety block after payment"
    );
    ...
    assert_eq!(rows[3]["payment"]["state"], "finalized");
    assert_eq!(rows[3]["decision"]["status"], "blocked");
    assert_eq!(rows[3]["decision"]["reason"], "intent_safety");
    assert_eq!(rows[3]["guardrails"]["state"], "blocked");
