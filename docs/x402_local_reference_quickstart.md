...
approved and one exit-4 blocked scenario. Both pass through the same
coordinator.
...
The blocked case records exit `4` and never calls or consumes the gate.

## Machine-checkable non-bypass result

For both scenarios the `authority` object keeps all eleven fields false,
