...
export const TESTNET_OUTCOME_CAPTURE_POSTMORTEM_SCHEMA_VERSION = 1 as const;
...
const LOSS_WINDOWS = Object.freeze([
  ...
  Object.freeze({
    code: "result_wrapper_not_durably_captured",
    reason:
      "the redacted diagnostic exists only in the returned execute result until the caller captures it",
  }),
  Object.freeze({
    code: "state_outcome_omits_diagnostic",
    reason:
      "the current state finalization boundary persists terminal outcome status but not the allowlisted diagnostic",
  }),
]);
...
export interface TestnetOutcomeCapturePostmortem {
  ...
  readonly returnedDiagnostic: Readonly<{
    stage: string;
    code: string;
  ...
  readonly captureBoundary: Readonly<{
    ...
    persistedOutcomeSurface: "state_port.finalize.outcome.status";
...
export function buildTestnetOutcomeCapturePostmortem(
  ...
  return Object.freeze({
    ...
    returnedDiagnostic: Object.freeze({
    ...
    captureBoundary: Object.freeze({
      ...
      persistedOutcomeSurface: "state_port.finalize.outcome.status" as const,
      terminalState: "outcome_unknown" as const,
      diagnosticPersisted: false as const,
