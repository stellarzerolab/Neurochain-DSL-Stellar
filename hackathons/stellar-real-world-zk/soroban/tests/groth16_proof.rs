...
#[test]
fn genuine_allowlist_block_proof_routes_as_exit_3() {
    let fixture: Groth16Fixture =
        serde_json::from_str(include_str!("../../fixtures/groth16_blocked_exit_3.json"))
            .expect("exit-3 Groth16 fixture must be valid JSON");
    let env = Env::default();
    env.mock_all_auths();
    ...
    assert_eq!(accepted.decision_status, DecisionStatus::Blocked as u32);
    assert_eq!(accepted.exit_code, ExitCode::Allowlist as u32);
    assert!(!accepted.requires_approval);
    assert_eq!(accepted.next_step, NextStep::Blocked);
    assert!(client.is_consumed(&accepted.audit_nullifier));
}
