...
To also write and validate a host-ready Windows MCP configuration with absolute
paths to the release server and local ONNX model, pass `-HostConfigOut`:
...
The gate reads the generated file back, checks that it contains only the stdio
executable path and `NC_INTENT_STELLAR_MODEL`, and launches the smoke harness
